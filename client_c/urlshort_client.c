...
int encurta(char *url_original, char *url_curta) {
  ...
  if (strncmp(resp, "OK ", 3) == 0) {
    strncpy(url_curta, resp + 3, CODE_MAX_LEN - 1);
...
const char *urlshort_strerror(int err) {
  ...
  case URLSHORT_OK:
    return "Sucesso";
  case URLSHORT_ERR_SOCKET:
    return "Falha ao criar socket";
  case URLSHORT_ERR_CONNECT:
    return "Falha ao conectar ao proxy";
  case URLSHORT_ERR_SEND:
    return "Falha ao enviar dados";
  case URLSHORT_ERR_RECV:
    return "Falha ao receber dados";
  case URLSHORT_ERR_PROTO:
    return "Resposta inesperada do proxy";
  case URLSHORT_ERR_ARG:
    return "Argumento invalido";
  case URLSHORT_ERR_SERVER:
    return "Proxy retornou erro";
  default:
